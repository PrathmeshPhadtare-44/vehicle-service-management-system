...
public VehicleController(VehicleServiceImpl vehicleService){
    ...
    public ResponseEntity<List<VehicleResponseDto>> findByCustomerId(@RequestParam long customerId){
    return ResponseEntity.ok(vehicleService.getVehiclesByCustomer(customerId));

    }
    @DeleteMapping("/{vehicleId}")
    public ResponseEntity<Void> deleteVehicle(@RequestParam long vehicleId){
